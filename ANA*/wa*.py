...
def search(map):
    ...
    wastar(map)
    print('WA* Search took {0} seconds with weight {1}'.format (time.time() - start_time,eps))
    print('Optimal Path Length {0} using L1 Heuristic'.format(len(path)))
    visualize_search("wa_e200_out.png") # see what your search has wrought (and maybe save your results)
    ...
    while not openlist.empty():
        ...
        for s1 in  neighbors:
            if g[s[0],s[1]]+1<g[s1[0],s1[1]]:
                ...
                parents[s1]=s
                openlist.delete(s1)
                openlist.put(s1,g[s1[0],s1[1]]+eps*heu(s1))
...
def heu(pt):
    return abs(pt[0]-end[1]+1)+abs(pt[1]-end[0]+1)
